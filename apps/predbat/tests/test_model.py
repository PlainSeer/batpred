...
def run_model_tests(my_predbat, prediction_kernel=False):
    ...
    # Freeze Export internal battery loss regression coverage. 240 W for one hour
    # is 0.24 kWh. The loss is battery-side only, must not apply outside Freeze
    # Export, must respect reserve, and is included in cycle accounting.
    failed |= simple_scenario(
        "freeze_export_loss_default_zero",
        my_predbat,
        0,
        0,
        ...
        discharge=99,
        end_record=60,
        inverter_freeze_export_loss=0.0,
        assert_battery_cycle=0.0,
    )
    failed |= simple_scenario(
        "freeze_export_loss_240w_one_hour",
        my_predbat,
        0,
        0,
        assert_final_metric=0,
        ...
        discharge=99,
        end_record=60,
        inverter_freeze_export_loss=240.0,
        assert_battery_cycle=0.24,
    )
    failed |= simple_scenario(
        "freeze_export_loss_not_outside_freeze",
        my_predbat,
        0,
        0,
        assert_final_metric=0,
    ...
    failed |= simple_scenario(
        "freeze_export_loss_reserve_floor",
        my_predbat,
        0,
        0,
        assert_final_metric=0,
